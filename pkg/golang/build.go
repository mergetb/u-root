...
// to `binaryPath`.
func (c Environ) Build(importPath string, binaryPath string) error {
	p, err := c.FindOneCmd(importPath)
	if err != nil {
		return err
	}
	return c.BuildDir(p.Dir, binaryPath)
}

// BuildDir compiles the package in the directory `dirPath`, writing the build
// object to `binaryPath`.
func (c Environ) BuildDir(dirPath string, binaryPath string) error {
	args := []string{
		"build",
